...
class ResidualAttentionBlock(nn.Module):
	...
	def forward(self, x_tuple:tuple):
		...
		# place 1, before the self-attention
		if self.deepcluster is not None:
			x, cluster_loss_tmp = self.deepcluster(x)
			cluster_loss += cluster_loss_tmp
...
class VisualTransformer(nn.Module):
	...
	def forward(self, x: torch.Tensor, video_frame=-1):
		...
		x, _, cluster_loss = self.transformer(x, video_frame=video_frame, visual=True)
...
def build_clip_model(state_dict: dict, convert_fp16=True, linear_patch='2d', cut_top_layer=0,
	...
	if args.cluser_embed_from_clip:
		log_info("[CLIP] Load new added cluster embedding from CLIP pretrained positional embedding...")
		for n, m in model.named_parameters():
			if 'cluster_embed' in n:
				log_info('[CLIP] Load params for {}'.format(n))
				i = m.shape[0]
				# m.data[0, ...].copy_(state_dict['visual.class_embedding'].data)
				# m.data[1:, ...].copy_(state_dict['visual.positional_embedding'].data[:i - 1, ...])
				# m.data.copy_(state_dict['visual.positional_embedding'].data[:i, ...])
				m.data.copy_(state_dict['visual.positional_embedding'].data[1:i + 1, ...])
			if 'cluster_cls_embed' in n:
				log_info('[CLIP] Load params for {}'.format(n))
				m.data[0, ...].copy_(state_dict['visual.class_embedding'].data)
		log_info("[CLIP] Load new added cluster embedding from CLIP pretrained positional embedding...[DONE]\n")
